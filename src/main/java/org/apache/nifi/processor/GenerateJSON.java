...
@SupportsBatching
@Tags({"test", "random", "generate", "json"})
@InputRequirement(Requirement.INPUT_FORBIDDEN)
@CapabilityDescription("This processor creates json FlowFiles with json data generated based on config provided. GenerateJSON is useful" +
...
public class GenerateJSON extends AbstractProcessor {
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) {
        ...
        if (context.getProperty(UNIQUE_FLOWFILES).asBoolean()) {
            ...
            String json_template = context.getProperty(CUSTOM_TEXT).evaluateAttributeExpressions().getValue();
            try {
                Map<String, Object> props = JSONConfigReader.readConfig(new ByteArrayInputStream(json_template.getBytes()), Map.class);
                ...
                json_output = mapper.writeValueAsString(map);
            } catch (IOException ex) {
                ex.printStackTrace();
            }
            data = json_output.getBytes();
